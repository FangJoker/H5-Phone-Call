...
public class WebRTCSigApi {
	...
	private static Logger logger = Logger.getLogger(WebRTCSigApi.class);
	 ...
	 * 
	 * @param expire
	 *            userSig有效期，出于安全考虑建议为300秒，您可以根据您的业务场景设置其他值。
	 * 
	 * @return 生成的userSig
	 * 
	 */
	 ...
	 * 
	 * @param expire
	 *            privateMapKey有效期，出于安全考虑建议为300秒，您可以根据您的业务场景设置其他值。
	 * 
	...
	/**	生成 userSig privateMapKey
	 * @param  roomId  房间号  , userId app中用户的唯一标识符
	 * @return Map   生成的 userSig privateMapKey
	 * **/
	public  Map<String, String> init (int roomId,String userId) {
		
		try{
			//BufferedReader in = new BufferedReader(new FileReader("C:/Users/59859/Workspaces/MyEclipse 2016 CI/liaoxueChat/src/main/resources/TRTC.properties"));
			BufferedReader in = new BufferedReader(new FileReader("/usr/apache-tomcat-8.0.51/webapps/liaoxueChat/WEB-INF/classes/TRTC.properties"));
			prop = new Properties();
			prop.load(in);
		}catch(IOException e){
			e.printStackTrace();
			logger.info("加载TRTC配置信息失败");
			
		}
		
		int sdkappid = Integer.parseInt(prop.getProperty("sdkAppId"))  ; // 腾讯云云通信sdkappid

		int roomid = roomId; // 音视频房间号roomid

		String userid = userId; // 用户名userid
		...
		String userSig = genUserSig(userid, 300);

		// 生成privateMapKey

		String privateMapKey = genPrivateMapKey(userid, roomid, 300);
		...
		result.put("privateMapKey",privateMapKey);
		return result;
